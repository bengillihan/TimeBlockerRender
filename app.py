...
def get_seven_day_stats():
    ...
    try:
        ...
        # Get daily plans for the date range
        daily_plans = DailyPlan.query.filter(
            DailyPlan.user_id == current_user.id,
            DailyPlan.date >= start_date,
            DailyPlan.date <= end_date
            ...
            for block in plan.time_blocks:
                if block.task_id:
                    ...
                    if task and task.category:
                        minutes = 15  # Each block is 15 minutes
                        total_minutes += minutes
                        
                        # Track APS/Work category specifically
                        if task.category.name.upper() == 'APS':
                            aps_minutes += minutes
                        ...
                        category_breakdown[cat_name]['minutes'] += minutes
        
        return jsonify({
            'success': True,
            'total_hours': round(total_minutes / 60, 1),
            'aps_hours': round(aps_minutes / 60, 1),
            'aps_progress_percent': min(100, round((aps_minutes / 60) / 32 * 100, 1)),
        ...
        logger.error(f"Error getting 7-day stats: {str(e)}")
...
def get_todos_list():
    ...
    try:
        ...
        # Calculate next occurrence for recurring todos
        next_occurrence = None
        if data.get('is_recurring') and data.get('recurrence_rule') and due_date:
            next_occurrence = calculate_next_due_date(due_date, data['recurrence_rule'])
...
def calculate_next_due_date(current_due_date, recurrence_rule):
    ...
    try:
        ...
        # Parse the recurrence rule and get the next occurrence
